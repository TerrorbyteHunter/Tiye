...
export const testResponsiveBreakpoints: TestBreakpoint[] = [
  {
    width: 375,
    height: 667,
    name: 'iPhone SE',
...
export function getCurrentBreakpoint(): TestBreakpoint | null {
  if (typeof window === 'undefined') return null;
  
  const currentWidth = window.innerWidth;
  const currentHeight = window.innerHeight;
  
  return testResponsiveBreakpoints.find(
    breakpoint => breakpoint.width === currentWidth && breakpoint.height === currentHeight
  ) || null;
}
