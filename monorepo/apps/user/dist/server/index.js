...
app.post('/api/user/sync', async (req, res) => {
    try {
        ...
        if (userResult.rows.length === 0) {
            // Insert new user
            await pool.query('INSERT INTO admins (firebase_uid, email, full_name, active, role) VALUES ($1, $2, $3, $4, $5)', [uid, email, displayName || '', true, 'staff']);
...
app.get('/api/user/routes/:routeId/seats', async (req, res) => {
    try {
        ...
        const bookedResult = await pool.query('SELECT seat_number FROM tickets WHERE route_id = $1 AND travel_date = $2', [routeId, travelDate]);
...
app.post('/api/user/google-auth', async (req, res) => {
    try {
        ...
        if (userResult.rows.length === 0) {
            // Create new user
            const insert = await pool.query('INSERT INTO users (firebase_uid, username, mobile, password_hash) VALUES ($1, $2, $3, $4) RETURNING *', [uid, email, '', '']);
            user = insert.rows[0];
        }
        else {
